...
function getHandValue(map: Map<string, number>) {
  const jValue = map.get("J") || 0;
  let highValue = 0;
  let hasPair = false;
  let hasThree = false;
  for(const key of map.keys()) {
    const val = map.get(key);
    //skip counting jokers
    if(key !== "J") {
      if(val === 3) {
        hasThree = true;
        if(hasPair) {
      ...
      highValue = Math.max(highValue, val || 0);
    }
  }
  // Can't have 5 of a kind and that tripped me up very hard until I checked against someone else's solution!
  return highValue + jValue
}
